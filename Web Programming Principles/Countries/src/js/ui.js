...
const ui = {
  ...
  countryCodeToFlagUrl: function (countryCode, size) {
    ...
    if (sizes.includes(size)) {
      url = `https://www.countryflags.io/${countryCode}/flat/${size}.png`;
    } else {
      throw 'Invalid image size. Must be 12, 24, 32, 48 or 64.';
    }
    return url;
  },
  ...
  countryCodeToImg: function (countryCode) {
    ...
    img.src = this.countryCodeToFlagUrl(countryCode, 64);
    return img;
  },
  ...
  countryToRow: function (country, language) {
    ...
    flag.appendChild(this.countryCodeToImg(country.code));
    flag.classList.add('center');
    tr.appendChild(flag);
    for (const key in country) {
      let row = document.createElement('td');
      if (key == 'name') {
        row.textContent = country[key];
        this.addLangToElement(row, language);
      } else {
        row.textContent = country[key];
      }
      tr.appendChild(row);
  ...
  countriesToTable: function (countries, language) {
    ...
    for (const con in countries) {
      table.appendChild(this.countryToRow(countries[con], language));
    }
  }
};
